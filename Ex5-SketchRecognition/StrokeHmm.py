...
class HMM:
    ...
    def label( self, data ):
        ...
        labelSequence = []
        partialProb = {}    # current partial probability
        priorPP = {}        # partial probability of last layer
        for i in range(len(data)):
            features = data[i]
            if i == 0:
                for s in self.states:
                    # partialProbability(state0) = P(state0) * P(feature|state0)
                    pp = self.priors[s] * self.getEmissionProb(s, features)
                    partialProb[s] = pp
            else:
                for s in self.states:
                    maxPP = -INF
                    for ps in self.states:
                        # partialProbability(state1) = priorPP * P(state1|state0) * P(feature|state1)
                        tempPP = priorPP[ps] * self.transitions[ps][s] * self.getEmissionProb(s, features)
                        # find the largest partial probability
                        if tempPP > maxPP:
                            maxPP = tempPP
                    partialProb[s] = maxPP
            
            # select the state with largest partial probability
            maxPP = -INF
            l = ''
            for s in self.states:
                if partialProb[s] > maxPP:
                    maxPP = partialProb[s]
                    l = s
            labelSequence.append(l)
            priorPP = partialProb.copy()
    ...
    def labelTester(self):
        ''' Part 1 Viterbi Testing Example'''
        self.featureNames = ['Dry', 'Damp', 'Soggy']
        self.featuresCorD = {'Dry': DISCRETE, 'Damp': DISCRETE, 'Soggy':DISCRETE}
        self.numVals = { 'Dry': 2, 'Damp': 2, 'Soggy': 2}

        self.states = ['Sunny', 'Rainy', 'Cloudy']
        self.priors = {'Sunny': 0.63, 'Cloudy': 0.17, 'Rainy': 0.20}
        self.transitions = {'Sunny': {'Sunny': 0.50, 'Cloudy': 0.25, 'Rainy': 0.25}, 
        'Cloudy': {'Sunny': 0.375, 'Cloudy': 0.125, 'Rainy': 0.375},
        'Rainy': {'Sunny': 0.125, 'Cloudy': 0.675, 'Rainy': 0.375}}
        self.emissions = {'Sunny': {'Dry': [0.40, 0.60], 'Damp': [0.85, 0.15], 'Soggy': [0.95, 0.05]},
        'Cloudy': {'Dry': [0.75, 0.25], 'Damp': [0.75, 0.25], 'Soggy': [0.75, 0.25]},
        'Rainy': {'Dry': [0.95, 0.05], 'Damp': [0.65, 0.35], 'Soggy': [0.50, 0.50]}}

        evidenceSequence = [{'Dry': 1}, {'Damp': 1}, {'Soggy': 1}]
        return self.label(evidenceSequence)
...
class StrokeLabeler:
    def __init__(self):
        ...
        self.featureNames = ['length', 'distance', 'curvature']
        self.contOrDisc = {'length': DISCRETE, 'distance': CONTINUOUS, 'curvature': CONTINUOUS}
        self.numFVals = { 'length': 2, 'distance': 2}

    def confusion(self, trueLabels, classifications):
        # initalize dictionary
        ret = {l: {k: 0 for k in self.labels} for l in self.labels}
        # compare each label in trueLabels with corresponding label from classificaions
        for i in range(len(trueLabels)):
            tLabel = trueLabels[i]
            cLabel = classifications[i]
            if tLabel == cLabel:
                ret[tLabel][tLabel] = ret[tLabel].get(tLabel, 0) + 1
            else:
                ret[tLabel][cLabel] = ret[tLabel].get(cLabel, 0) + 1
        return ret
    ...
    def labelFile( self, strokeFile, outFile ):
        ...
        print "Confusion matrix is:", self.confusion(trueLabels, labels)
...
class Stroke:
    ...
    def distanceOfSide(self):
        ''' Returns the distance from side of sketch '''
        minX, minY =  INF, 0.0
        maxX, maxY = -INF, 0.0
        # find the lefest point and rightest point
